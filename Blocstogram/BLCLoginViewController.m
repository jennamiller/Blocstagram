...
//  Blocstogram
//
//  Created by Jenna Miller on 12/27/14.
...
- (void)viewDidLoad {
    ...
    NSString *urlString = [NSString stringWithFormat:@"http://instagram.com/oauth/authorize/?client_id=%@&redirect_uri=%@&response_type=token", [BLCDataSource instagramClientID], [self redirectURI]];
...
- (BOOL) webView:(UIWebView *)webView shouldStartLoadWithRequest:(NSURLRequest *)request navigationType:(UIWebViewNavigationType)navigationType {
    ...
    if ([urlString hasPrefix:[self redirectURI]]) {
        NSRange rangeOfAccessTokenParamater = [urlString rangeOfString:@"access_token="];
        NSUInteger indexOfTokenStarting = rangeOfAccessTokenParamater.location + rangeOfAccessTokenParamater.length;
